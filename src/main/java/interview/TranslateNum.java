...
public class TranslateNum {
	...
	private static String postCheck(StringBuilder temp) {
		...
		int i = arr.length - 1 ;
		for(; i>0; i--){
			if(!Character.valueOf(arr[i]).toString().equals("零")){ // char vs string
				break;
			}
		}
	...
	private static void init() {
		map.put(0+"", "零");
		map.put(1+"", "一"); map.put(2+"", "二"); map.put(3+"", "三"); map.put(4+"", "四"); map.put(5+"", "五");
		map.put(6+"", "六"); map.put(7+"", "七"); map.put(8+"", "八"); map.put(9+"", "九");
	}
