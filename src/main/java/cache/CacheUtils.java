...
public class CacheUtils {
	...
	public static void set(String key, Object data, long time, TimeUnit unit) {
		...
		switch (unit) {
			case DAYS:
				expireTime = currentTime + time * 24 * 60 * 60 * 1000;
				break;
			case HOURS:
				expireTime = currentTime + time * 60 * 60 * 1000;
				break;
			case MINUTES:
				expireTime = currentTime + time * 60 * 1000;
				break;
			case SECONDS:
				expireTime = currentTime + time * 1000;
				break;
			default:
				throw new UnsupportedOperationException("暂不支持此级别缓存");
		}
		set(key, data, expireTime);
	}
